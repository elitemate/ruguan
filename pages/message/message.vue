<template>
    ...
    <view class="index-content" style="background: white">
        ...
        <view class="index-coupon has-bg-white has-pd-10">
            <view class="goods-list" v-if="message.length > 0&&showmessage" >
                <orange-message
                        v-for="(h,index) in message"
        ...
        <view class="s-col is-col-24" v-if="message.length > 0||news.length>0">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            messageOnLoad() {
                this.getmessage(this.messageLoad.total += 1);
            },
            getmessage(page) { //获取精选商品数据
                this.showNews = false;
                this.showmessage = true;
                this.loadingType = 1;
                this.$Request.get('/selected_item/apikey/maxd/min_id/' + this.min_id).then(res => {
                    ...
                    if (res.code === 1) {
                        ...
                        this.messageLoad.loading = false;
                    } else {
                        this.loadingType = 2;
                        this.messageLoad.loading = false;
                        this.messageLoad.finished = true;
                    }
                    uni.hideLoading();
