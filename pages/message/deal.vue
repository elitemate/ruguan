<template>
    ...
    <view class="index-content" style="background: white">
        ...
        <view class="index-coupon has-bg-white has-pd-10">
            <view class="goods-list" v-if="deal.length > 0&&showdeal" >
                <orange-deal
                        v-for="(h,index) in deal"
       ...
       <!-- <view class="scroll_top" @tap="topScrollTap" v-bind:class="[scrollTop ? 'active' : '','']"
        ...
        <!-- 加载更多提示 -->
        <view class="s-col is-col-24" v-if="deal.length > 0||news.length>0">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            dealOnLoad() {
                this.getdeal(this.dealLoad.total += 1);
            },
            getdeal(page) { //获取精选商品数据
                this.showNews = false;
                this.showdeal = true;
                this.loadingType = 1;
                this.$Request.get('/selected_item/apikey/maxd/min_id/' + this.min_id).then(res => {
                    ...
                    if (res.code === 1) {
                        ...
                        this.dealLoad.loading = false;
                    } else {
                        this.loadingType = 2;
                        this.dealLoad.loading = false;
                        this.dealLoad.finished = true;
                    }
                    uni.hideLoading();
