    ...
    <view>
        <view style="background: white;z-index: 999">
            <scroll-view scroll-x class="bg-white nav" scroll-with-animation :scroll-left="scrollLefts"
                         style="width: 100%;position: fixed;z-index: 100;text-align: center;border-bottom:1px solid ghostwhite">
            ...
            <view class="index-coupon has-bg-white has-pd-10 top_30">
                <view class="goods-list" v-if="couponlist.length > 0">
                    <orange-goods-card-home
                            ...
                            :itemid="item.itemid"
                            :itempic="item.itempic?item.itempic+'_310x310.jpg':'../../static/img/avatar.png'"
    ...
    export default {
        ...
        methods: {
            ...
            loadCouponList: function (type) {
                this.loadingType = 1;
                let that = this;
                this.$Request.get('/column/apikey/maxd/type/' + this.type + '/back/20/sort/' + this.sort + '/min_id/' + this.min_id + '/cid/11').then(res => {
                    this.loadingType = 0;
                    if (res.code === 1) {
                        if (this.page === 1) {
                            this.couponlist = [];
                        ...
                        that.$Request.get("/tao/wx/js/sign?url=" + url).then(res => {
                            ...
                            that.$wx.ready(function () {
                                ...
                                that.$wx.onMenuShareAppMessage({
                                    ...
                                    imgUrl: 'https://www.gomyorder.cn/shg.png', // 分享图标
